...
void LCD::LCD_print_string(std::string word,uint8_t line, uint8_t column)
{
    LCD_cursor_location(line, column);
...
void LCD::LCD_cursor_location(uint8_t line,uint8_t column)
{
    if(line == 0 )
    {
     LCD_Send_Command_or_Char(0b10000000 +column,0);//Sets address to 0x00 with column offset
    }
    else  {
        LCD_Send_Command_or_Char(0xC0+column,0); //sets address to 0x40 with column offset
    }
